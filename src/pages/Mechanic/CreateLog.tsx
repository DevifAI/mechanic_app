...
const CreateLog = () => {
  ...
  const handleSave = async () => {
    try {
      console.log('Saving log with:', { date, logNumber, equipment, note, nextDate, actionPlan, items });
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              await AsyncStorage.setItem('logItems', JSON.stringify(updatedItems));
  ...
  return (
    ...
    >
        ...
        <Text  style={{ color: '#007AFF', fontWeight: 'bold', marginBottom: 2 , fontSize:16 }}>
          Maintenance Log Number <Text style={{ color: 'red', fontSize:16 }}>*</Text></Text>
        <TextInput
          style={styles.input}
        ...
        {showEquipDropdown && (
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity style={styles.dropdownItem} onPress={() => handleEquipSelect(item)}>
                <Text>{item.name}</Text>
              </TouchableOpacity>
            )}
          />
...
{showNextDatePicker && (
  ...
  />
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('AddItem', { existingItems: items, targetScreen: 'CreateLog' })}
        >
