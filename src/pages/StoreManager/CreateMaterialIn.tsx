...
const CreateMaterialIn = () => {
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const isFocused = useIsFocused();

  const [items, setItems] = useState<MaterialInItem[]>([]);
  ...
  useEffect(() => {
    const mergeItems = async () => {
      if (isFocused && route.params?.updatedItems) {
        ...
        try {
          const stored = await AsyncStorage.getItem('MaterialInItems');
          const parsedStored: MaterialInItem[] = stored ? JSON.parse(stored) : [];

          const merged = [...parsedStored];

          newItems.forEach((newItem: MaterialInItem) => {
          ...
          setItems(merged);
          await AsyncStorage.setItem('MaterialInItems', JSON.stringify(merged));
  ...
  const handleSave = async () => {
    try {
      ...
      await AsyncStorage.removeItem('MaterialInItems');
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const storedItems = await AsyncStorage.getItem('MaterialInItems');
              const parsedItems = storedItems ? JSON.parse(storedItems) : [];

              const updatedItems = parsedItems.filter((item: any) => item.id !== id);

              await AsyncStorage.setItem('MaterialInItems', JSON.stringify(updatedItems));
  ...
  const renderItem = ({ item, index }: { item: MaterialInItem; index: number }) => (
    ...
    >
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Create Material In</Text>
          <TouchableOpacity
            onPress={handleSave}
            style={{
        ...
        <TextInput 
          ...
          placeholderTextColor="#A0A0A0"
          style={styles.input} value={challanNo} onChangeText={setChallanNo} 
        />
...
export default CreateMaterialIn;
