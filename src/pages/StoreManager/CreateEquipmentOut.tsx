...
const CreateEquipmentOut = () => {
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const isFocused = useIsFocused();

  const [items, setItems] = useState<EquipmentOutItem[]>([]);
  ...
  useEffect(() => {
    const mergeItems = async () => {
      if (isFocused && route.params?.updatedItems) {
        ...
        try {
          const stored = await AsyncStorage.getItem('equipmentOutItems');
          const parsedStored: EquipmentOutItem[] = stored ? JSON.parse(stored) : [];

          const merged = [...parsedStored];

          newItems.forEach((newItem: EquipmentOutItem) => {
          ...
          setItems(merged);
          await AsyncStorage.setItem('equipmentOutItems', JSON.stringify(merged));
  ...
  const handleSave = async () => {
    try {
      ...
      await AsyncStorage.removeItem('equipmentOutItems');
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              const storedItems = await AsyncStorage.getItem('equipmentOutItems');
              const parsedItems = storedItems ? JSON.parse(storedItems) : [];

              const updatedItems = parsedItems.filter((item: any) => item.id !== id);

              await AsyncStorage.setItem('equipmentOutItems', JSON.stringify(updatedItems));
  ...
  const renderItem = ({ item, index }: { item: EquipmentOutItem; index: number }) => (
    ...
    >
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Create Equipment Out</Text>
          <TouchableOpacity
            onPress={handleSave}
            style={{
        ...
        {showDatePicker && (
         ...
         <Text style={styles.label}>Reason Out</Text>
                <TextInput
...
export default CreateEquipmentOut;
