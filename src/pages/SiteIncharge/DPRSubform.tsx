...
const DPRSubform = () => {
  ...
  const {fetchRevenueList, revenue} = useSuperadmin();
  ...
  const [revenueCode, setRevenueCode] = useState(
    editingItem?.revenueCode || '',
  );
  const [revenueCodeId, setRevenueCodeId] = useState(
    editingItem?.revenueCodeId || '',
  );
  const [filteredRevenueCodes, setFilteredRevenueCodes] = useState(revenue);
...
const handleRevenueCodeChange = (text: string) => {
  ...
  // Save handler
  const handleSave = () => {
    if (!timeFrom || !timeTo || !jobDone || !revenueCode) {
