...
const [showEndTimePicker, setShowEndTimePicker] = useState(false);
  ...
  const dummyShiftCodes = ["ShiftM-101", "ShiftM-102", "ShiftA-201", "ShiftB-301"];
    ...
    const mergeItems = async () => {
      if (isFocused && route.params?.updatedItems) {
        ...
        try {
          ...
          const parsedStored: DPRSubformItem[] = stored ? JSON.parse(stored) : [];
    ...
    Alert.alert('Delete Item', 'Are you sure you want to delete this item?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          const parsedItems: DPRSubformItem[] = storedItems ? JSON.parse(storedItems) : [];
    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
        <View style={styles.header}>
          <TouchableOpacity 
          onPress={() => navigation.navigate('MainTabs', { screen: 'DprScreen' })}
...
/>
        ...
        style={styles.input} value={customerRep} onChangeText={setCustomerRep} />
...
>
  {/* Label */}
  <Text style={{ color: '#007AFF', fontWeight: 'bold', marginBottom: 12, fontSize: 16 }}>
    Shift Start Time <Text style={{ color: 'red', fontSize: 16 }}>*</Text>
  </Text>

  {/* Time + Icon */}
  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
    <Text style={{ fontSize: 16, color: shiftStartTime ? '#000' : '#A0A0A0' }}>
...
{showStartTimePicker && (
  <DateTimePicker
    value={shiftStartTime ? parseTimeToDate(shiftStartTime) : new Date()}
    mode="time"
    is24Hour={false}
    display="spinner"
...
>
  {/* Label */}
  <Text style={{ color: '#007AFF', fontWeight: 'bold', marginBottom: 8, fontSize: 16 }}>
    Shift End Time <Text style={{ color: 'red', fontSize: 16 }}>*</Text>
  </Text>

  {/* Time + Icon */}
  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
    <Text style={{ fontSize: 16, color: shiftEndTime ? '#000' : '#A0A0A0' }}>
...
{showEndTimePicker && (
  ...
  />
        ...
        <TextInput 
         ...
         placeholderTextColor="#A0A0A0"
        style={styles.input} value={shiftIncharge} onChangeText={setShiftIncharge} />

        <Text style={styles.label}>Shift Mechanic</Text>
        <TextInput 
         placeholder="Enter Shift Mechanic"
         placeholderTextColor="#A0A0A0"
        style={styles.input} value={shiftMechanic} onChangeText={setShiftMechanic} />
