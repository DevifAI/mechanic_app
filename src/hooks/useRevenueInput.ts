  ...
  const createRevenueInputById = async (
    ...
    try {
      const response = await createRevenueInput(payload, role as Role);
        Toast.show({
      type: 'success',
      text1: 'Revenue Record Created successfully',
    });
      if (callback) callback();
    } catch (error: any) {
      console.error('Error creating Revenue record:', error?.data?.message);
        Toast.show({
      type: 'error',
      text1: 'Creation Failed',
      text2: error?.data?.message || 'Something went wrong while creating the revenue record.',
    });
    } finally {
      setLoading(false);
    }
